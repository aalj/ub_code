...
public class DBProvider extends ContentProvider{
	...
	static{
		...
		URIMATCHER.addURI(DatabaseColumn.AUTHORITY, AlarmColumn.TABLE_NAME, 1);
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		case ALARM:
			cursor = query(AlarmColumn.TABLE_NAME, projection,selection, selectionArgs, "", "", sortOrder);
			break;
		}
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		switch(witch){
		case ALARM:
			rowId=insert(AlarmColumn.TABLE_NAME,null,values);
			break;
		}
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		switch(witch){
		case ALARM:
			count=delete(AlarmColumn.TABLE_NAME, selection, selectionArgs);
			notifyChange(uri);
			break;
		}
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		switch(URIMATCHER.match(uri)){
		case ALARM:
			count = update(AlarmColumn.TABLE_NAME,values,selection,selectionArgs);
			notifyChange(uri);
			break;
		}
