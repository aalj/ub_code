package com.tz.mvp.framework.support.delegate.viewgroup;
...
public class OrientationChangeManager<V extends MvpView, P extends MvpPresenter<V>> {
	...
	/**
	 * Determines if the view will be destroyed permanently, because the whole
	...
	/**
	 * Determines if the view will be detached from window (destroyed) because
	 * of an orientation change
	 *
	 * @param context
	 *            the context
	 * @return true, if detached because of an orientation change. Otherwise,
	 *         false
	 */
	...
	/**
	 * Remove the Presenter and ViewState from internal
	 * {@link OrientationChangeFragment}
	 *
	 * @param viewId
	 *            The (internal) view's id
	 * @param context
	 *            The context
	 */
	...
	/**
	 * Puts the presenter in the internal cache "associated" with the given view
	 * id
	 *
	 * @param viewId
	...
	public static final class OrientationChangeFragment extends Fragment {
		 ...
		 * @param <T>
		 *            The retrurn type
		 * @return the cache entry. Is <code>null</code> if no entry found for
		 *         the given key
		 */
		<T> T get(int key) {
